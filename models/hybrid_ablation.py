...
class StaticFusionHybridModel(nn.Module):
    """Hybrid ablation without learned target-wise gating."""
    ...
    def forward(self, x):
        ...
        base_prediction = 0.5 * (bilstm_pred + tft_pred)
        residual_correction = self.fusion_refiner(torch.cat([fused_context, bilstm_pred, tft_pred], dim=-1))
        final_prediction = base_prediction + residual_correction
