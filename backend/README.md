...
The production stack runs the React frontend, Node backend, PostgreSQL, and Redis:

```bash
cp backend/.env.production.example backend/.env.production
# set CLIENT_ORIGIN, DB passwords, and any provider keys (WAQI/OpenWeather/Google)
GROQ_API_KEY=... GEMINI_API_KEY=... docker compose up --build -d
```
...
```text
web       nginx + built React app on port 80 (proxies /api -> app:5000)
app       Node.js Express API on port 5000
postgres  PostgreSQL 16 with persistent volume
redis     Redis 7 with append-only persistence
```

The backend image is built from the repository root (`context: .`) and bakes the small model
artifacts it serves (`outputs/past_present_future_forecast.csv`, `outputs/forecast_2026.csv`,
`outputs/urban_events.json`, `outputs/urban_anomaly_timeline.csv`) into `/app/outputs`, with
`PROJECT_ROOT=/app`. If you regenerate those artifacts after a new training run, rebuild the
backend image to refresh them.

Model forecast live mode: with `MODEL_FORECAST_LIVE=true` (default) the `/api/model/forecast` and
`/api/model/conditions` endpoints re-anchor future-row timestamps to the server clock
(`now + step_ahead * MODEL_FORECAST_STEP_MINUTES`), so the dashboard always shows a rolling
"today -> tomorrow" outlook instead of the frozen training-end date.

2026 long-range forecast: `/api/model/forecast-yearly?year=2026` returns the full-year outlook
(monthly aggregates + hourly series) generated by `generate_2026_forecast.py` from the trained
hybrid model and the 2022–2025 history. Regenerate the artifact with:

```bash
python3 generate_2026_forecast.py
```

Production defaults include non-root app execution, dependency health checks, restart policies,
internal-only PostgreSQL/Redis networking, log rotation, and `no-new-privileges`.
