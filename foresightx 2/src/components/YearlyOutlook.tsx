  ...
  return (
    ...
    >
      ...
      <header className="flex justify-between items-end flex-wrap gap-6 mb-2">
        <div>
          ...
          <div style={{ fontSize: 14, color: '#6f8079', marginTop: 8, maxWidth: 560, lineHeight: 1.5 }}>
            ...
            history. Monthly means below, with hourly detail available in the year timeline.
          </div>
        </div>
      ...
      <div className="grid mt-4 gap-4" style={{ gridTemplateColumns: 'repeat(3, 1fr)' }}>
        {[
          { title: 'Peak AQI month', label: peaks.peakAqiMonth?.label, value: peaks.peakAqiMonth?.aqi, unit: 'avg AQI', color: COLORS.aqi },
          { title: 'Hottest month', label: peaks.hottestMonth?.label, value: peaks.hottestMonth?.temperature, unit: 'avg °C', color: COLORS.temperature },
          { title: 'Peak traffic month', label: peaks.peakTrafficMonth?.label, value: peaks.peakTrafficMonth?.trafficFlow, unit: 'avg veh/hr', color: COLORS.traffic },
        ].map((m) => (
